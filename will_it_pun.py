...
phoneme_file = "data/phoneme_lists/phoneme_dict"
...
class PhonemeDictionary:
	...
	def __init__(self, phoneme_dict_file):
		...
		with open(phoneme_dict_file) as f:
			for line in f:
				word_and_phoneme = line.split("\t")
				word = self._normalize_word(word_and_phoneme[0])
				phoneme = self._normalize_phoneme(word_and_phoneme[1])
